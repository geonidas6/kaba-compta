...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FAF8F5]">
      ...
      >
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div onClick={() => navigate("/")} className="flex items-center gap-2 cursor-pointer select-none" title="Aller à la page d'accueil">
            ...
            <div>
              <div className="font-['Manrope'] font-bold text-[#2D2D2D] leading-none">
                Kaba-Compta
              </div>
              <div className="text-xs text-[#6C6C6C] leading-tight">
                {user?.shop_name || user?.display_name} · {roleLabel}
              </div>
            </div>
          </div>
          <button
      ...
      <nav
        className="fixed bottom-0 left-0 right-0 z-30 bg-white/95 backdrop-blur border-t border-[#EAE5D9] safe-bottom"
